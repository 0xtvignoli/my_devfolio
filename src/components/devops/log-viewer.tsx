  ...
  return (
    <Card className="flex flex-col h-[55vh] min-h-[400px]">
      <CardHeader>
        ...
        <CardDescription>
          {isScenarioLog
            ? 'Live logs from the AI-generated pipeline run.'
            : 'Simulated shell for interacting with the production environment.'}
        </CardDescription>
      </CardHeader>
        ...
        <ScrollArea className="flex-1 border rounded-md bg-background/50" viewportRef={viewportRef}>
          <div className="p-4 font-mono text-sm space-y-1">
            {displayedLogs.map((log, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:gap-x-4">
                {log.command === 'prompt' ? (
                  ...
                  <>
                    ...
                    <span className={cn('font-bold', levelStyles[log.level])}>[{log.level}]</span>
