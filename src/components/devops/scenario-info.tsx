        ...
        {scenario.isIncident && pipelineStatus === 'failure' && !fixApplied && (
          <div className="space-y-3">
            <div className="text-sm text-muted-foreground">
              Pipeline failed. Choose a debug path:
            </div>
            <div className="flex flex-col gap-2">
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={scenarioDebugMode === 'manual'}
                onClick={() => {
                  startScenarioDebug('manual');
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={scenarioDebugMode === 'auto'}
                onClick={() => startScenarioDebug('auto')}
              >
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="text-xs font-mono max-h-44 overflow-auto pr-1 custom-scrollbar">
                    Hint {currentHintIndex + 1} / {scenarioHints.length}:{' '}
                    {scenarioHints[currentHintIndex] || 'Loading...'}
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    <Button
                      size="sm"
                      variant="secondary"
                      disabled={currentHintIndex >= scenarioHints.length - 1}
                      onClick={nextHint}
                    >
                      Next
                    </Button>
                    <Button size="sm" onClick={applyScenarioFix}>
