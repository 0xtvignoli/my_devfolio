...
> = {
  OPERATIONAL: {
    className: "bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:border-green-700/40 dark:text-green-300",
    icon: <CheckCircle className="h-3 w-3" />,
  },
  DEGRADED: {
    className: "bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/30 dark:border-yellow-700/40 dark:text-yellow-300 animate-pulse",
    icon: <AlertTriangle className="h-3 w-3" />,
  },
  OUTAGE: {
    className: "bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:border-red-700/40 dark:text-red-300 animate-pulse",
            ...
            <AccordionItem value={`item-${index}`} key={service.id}>
              ...
              <AccordionContent>
                <div className="space-y-2 pl-6 border-l-2 ml-2 border-dashed">
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Pods ({service.pods.length})</h4>
